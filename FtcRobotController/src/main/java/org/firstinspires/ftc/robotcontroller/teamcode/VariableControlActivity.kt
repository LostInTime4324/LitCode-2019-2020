...
class VariableControlActivity : Activity() {
    ...
    fun addViews(first: View, second: View, row: Int) {
        findViewById<GridLayout>(R.id.variableControlLayout).addView(first, createGridParams(row, 0))
        findViewById<GridLayout>(R.id.variableControlLayout).addView(second, createGridParams(row, 1))
    }
}