...
@Autonomous(name="EncoderNavTest", group="Pushbot")
...
public class EncoderNavTest extends LinearOpMode {
    ...
    static final double P_TURN_COEFF = 0.4;     // Larger is more responsive, but also less stable
...
//        }
        ...
        if (frontLeftDrive.getCurrentPosition() <= 0 && frontLeftDrive.getCurrentPosition() >= -300) {

            distance = -NumberVariable.FIRST_DISTANCE_TO_WAFFLE.getNumber() + 20;

            Stone = "first";

        } else if (frontLeftDrive.getCurrentPosition() >= -600 && frontLeftDrive.getCurrentPosition() < -300) {

            distance = -NumberVariable.SECOND_DISTANCE_TO_WAFFLE.getNumber() + 20;

            Stone = "Second";

        } else if (frontLeftDrive.getCurrentPosition() < -600) {
